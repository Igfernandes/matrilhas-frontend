...
import { getAgenciesCharges } from "@services/Agencies/Charges/Get/SSR";
...
export const getServerSideProps: GetServerSideProps<ChargePageProps> = async ({
  ...
  const { rows: charges } = await getAgenciesCharges(tokenNavigation, { id: parseInt(id) });

  if (!Array.isArray(charges) || charges.length === 0) {
    return {
      redirect: {
        destination: `${privateRoutes.panel.charges}?alert=${i18n(
