...
export function TourLanding({ tour }: Props) {
    const { t } = useI18n()
    const { formMethods,
        handleSubmit,
        onSubmit,
        ...
        <FormProvider {...formMethods}>
            <form className="bg-white w-full px-4 py-1 shadow-sm" onSubmit={handleSubmit(onSubmit)}>
                <div className="mt-5 mb-3">
                    <h2 className="text-3xl font-semibold text-primary">{t("Screens.dashboard.tours.address.landing.title")}</h2>
                    <p className="text-sm">{t("Screens.dashboard.tours.address.landing.description")}</p>
                </div>
                    ...
                    <Skeleton isLoading={isLoadingAddresses} settings={{
                        ...
                        {Array.from({ length: amount }).map((_, index) => (
                            <div key={`boarding_${index}_address`} className="bg-white inline-block mx-auto my-1 px-4 rounded-md shadow-sm">
                                <Accordion >
                                    <LandingItem id={index} onRemove={handleRemoveAddress} />
                                </Accordion>
                            </div>
                        ))}
                    </Skeleton>
